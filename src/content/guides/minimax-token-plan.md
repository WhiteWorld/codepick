...
tags: ["minimax", "token-plan", "coding-plan", "cline", "claude-code", "cursor", "M2.7", "订阅", "api"]
pillar: plans
content_status: keep
...
MiniMax 近期将原有的 **Coding Plan 升级为 Token Plan**，这不仅仅是改个名字——底层模型升级到 M2.7、额度大幅增加，文本模型从 prompt 口径改为 request 口径，并把文本、语音、视频、图像、音乐都纳入同一套 Token Plan / Credits 资源体系。对于使用 [Cline](/zh/tool/cline)、[Claude Code](/zh/tool/claude-code)、[Cursor](/zh/tool/cursor) 等工具的国内开发者来说，这是一个值得重新评估的方案。
